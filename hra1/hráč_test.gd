...
var attack_in_progress = false  # Přidáme flag pro útok

func _ready():
	# Opravený způsob připojení signálu pro Godot 4
	ap.animation_finished.connect(self._on_animation_finished)
...
func update_Animation(direction):
	# Pokud je útok aktivní a není ještě dokončen, neprováděj žádné jiné animace
	if attack_in_progress:
		return

	# Pokud je stisknutý tlačítko pro útok, spusť animaci
...
# Tento callback bude vyvolán, když animace skončí
func _on_animation_finished(anim_name: String) -> void:
	if anim_name == "attack":
		print("Attack animation finished")
		attack_in_progress = false  # Po dokončení útoku povolíme další animace
